...
happyExpList = Happy_Data_Array.listArray (0,28) ([3648,16,256,0,0,0,512,0,912,0,61,36864,3,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseMDL","Expr","ExprList","Conditional","if","then","else","forward","rotateleft","rotateright","obstacle","%eof"]
        bit_start = st * 14
        bit_end = (st + 1) * 14
...
action_13 (4) = happyGoto action_10
...
action_14 (4) = happyGoto action_12
...
happyReduction_2 (HappyTerminal (TokenForward happy_var_1))
	 =  HappyAbsSyn4
		 (Forward happy_var_1
	)
...
happyReduction_6 (HappyAbsSyn4  happy_var_2)
	(HappyAbsSyn5  happy_var_1)
	 =  HappyAbsSyn5
		 (ExprList happy_var_1 happy_var_2
	)
happyReduction_6 _ _  = notHappyAtAll 

happyReduce_7 = happySpecReduce_1  6 happyReduction_7
happyReduction_7 (HappyTerminal (TokenObstacle happy_var_1))
	 =  HappyAbsSyn6
		 (Obstacle happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenThen -> cont 8;
	TokenElse -> cont 9;
	TokenForward happy_dollar_dollar -> cont 10;
	TokenRotateLeft -> cont 11;
	TokenRotateRight -> cont 12;
	TokenObstacle happy_dollar_dollar -> cont 13;
	_ -> happyError' ((tk:tks), [])
	}
